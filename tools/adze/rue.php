...
Class rs_api {
    ...
    public function get_player_account_type($player_name) {

     $hcim_check = $this->check_hcim($player_name);
      if(!$hcim_check){
          $im_check = $this->check_im($player_name);
          if(!$im_check){
              $type = "Normal";
          }else{
              $type = "Ironman";
    ...
    private function check_im($player_name) {
        $response = $this->get_raw('http://services.runescape.com/m=hiscore_ironman/index_lite.ws?player='.$this->normalize_name($player_name));
        if (!$response) {
            return false;
        } else {
            return true;
    ...
    private function check_hcim($player_name) {
        $response = $this->get_raw('http://services.runescape.com/m=hiscore_hardcore_ironman/index_lite.ws?player='.$this->normalize_name($player_name));
        if (!$response) {
            return false;
        } else {
            return true;
