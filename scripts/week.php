...
<?php
	...
	function parse_function($player, $raw) {
		...
		/* Reset week */
		reset_week($player, $stats, $link, $f2p_skills_with_ehp);
	...
	function reset_week($player, $stats, $link, $f2p_skills_with_ehp) {
		/* update week */
		foreach($f2p_skills_with_ehp as $skill) {
			...
			$link->query("UPDATE week_stats SET $skill_xp = $xp WHERE rsn='$player'");
		}

		/* reset gains */
		$link->query("UPDATE week SET
			total_xp=0,
			attack_xp=0,
			defence_xp=0,
			strength_xp=0,
