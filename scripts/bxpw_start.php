...
<?php
	...
	function delete_user($player) {
		...
		$link->query("DELETE FROM bxpw WHERE rsn='$player'");
		$link->query("DELETE FROM bxpw_stats WHERE rsn='$player'");
	...
	function reset_bxpw($player, $stats, $link, $f2p_skills_with_ehp) {
		...
		foreach($f2p_skills_with_ehp as $skill) {
			...
			$link->query("UPDATE bxpw_stats SET $skill_xp = $xp WHERE rsn='$player'");
		}

		/* reset BXPW gains */
		$link->query("UPDATE bxpw SET
			total_xp=0,
			attack_xp=0,
			defence_xp=0,
			strength_xp=0,
